...
// Use Case:
// On the Airport, we have multiple check in counters, where traveller ticket is scanned may 
// be and we put the entry that traveler has checked in
...
class AirlineTicket{
	...
	AirlineTicket(String pnr, String name) {
		this.pnr = pnr;
		this.name = name;
		seatNumber = 0;
		checkedIn = false;
...
/*
	...
	//		Checking the User in may be a long running operation and hence we have done it through threads
			...
			System.out.println("Check in process finished: "+ticket.name+" has been allocated seat#"+ticket.seatNumber);
		}
	}
*/
...
//		We will create 5 threads and all of them shall be executed parallelly
//		If we execute n-number of threads all together, we are making available processors to be occupied fully
		
//		We can create a Thread Pool - which will take number of cores and will work accordingly for efficient usage
		ExecutorService service = Executors.newFixedThreadPool(2);  // 2 must be a number as compared to what you have in overall available processors
...
//		Now we will use only 2 processors i.e. cores to execute multiple threads
		...
		Callable<String> task1 = new CheckInTask(ticket1);
		Callable<String> task2 = new CheckInTask(ticket2);
		Callable<String> task3 = new CheckInTask(ticket3);
		Callable<String> task4 = new CheckInTask(ticket4);
		Callable<String> task5 = new CheckInTask(ticket5);
